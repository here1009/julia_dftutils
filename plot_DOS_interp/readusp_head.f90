subroutine readusp_head(filename,iiatom,nref, z_type,Ecut_type,amass_type)
    ...
    dimension beta(idim1,idim3),qfunc(idim1,idim3,idim3),qfcoef(idim8,idim6,idim3,idim3),vloc(idim1),vloc0(idim1)
    !c.....indexing for the beta functions
    dimension nbl0(idim5),nbl(idim5)
    !c.....angular momenta, reference energy, qij and Dij_8 of vanderbilt scheme
    dimension lll(idim3),eee(idim3),iptype(idim3),qqq(idim3,idim3),ddd0(idim3,idim3),ddd(idim3,idim3)
    ...
    !cc     +    form = 'unformatted' )
    open(unit=iops, file = filename, status = 'old', action='read',iostat=ierr)
    read (iops,*) (iver(i),i=1,3),(idmy(i),i=1,3)
    if (iver(1) .gt. 99 .or. iver(1) .lt. 1 .or. iver(2) .gt. 9  .or. iver(2) .lt. 0 .or. iver(3) .gt. 9  .or. iver(3) .lt. 0) then
        ...
        90     format('pseudopotential program version',i3,'.', i1,'.',i1,'   date:',i3,' - ',i2,' - ',i4)
        rewind (iops)
    endif
    !c
    !c       set the mesh type
    ...
    !c
    !c
    read(iops,*) title,zps,zvps,exftps,nvalps,mesh,etot,ecut_t,amass_t
    iiatom=zps*1.001
    z_type=zvps
    Ecut_type=ecut_t
    amass_type=amass_t
    ...
    do j=1,nbeta
        ...
        nref=nref+lll(j)*2+1
        do k=j,nbeta
            read (iops,*) ddd0(j,k),ddd(j,k),qqq(j,k),(qfunc(i,j,k),i=1,kkbeta),((qfcoef(i,lp,j,k),i=1,nqf),lp=1,nlc)
            ddd0(k,j)=ddd0(j,k)
            ddd (k,j)=ddd (j,k)
            qqq (k,j)=qqq (j,k)
            do i=1,kkbeta
